...
- 데이터에 없는 수치·국가는 지어내지 않고 "답변할 수 없다"고 말한다 (이번 메시지에 실제로 첨부된 데이터 블록만 근거로 삼고, 이전 turn이나 규칙 설명에만 등장한 데이터 종류는 이번에 없으면 없는 것으로 취급 — [12.10](#1210-대화-히스토리-도입-후-없는-데이터-종류를-형식만-보고-지어내는-문제) 참고)
...
- JSON 키·테이블명뿐 아니라 데이터 값 안에 섞인 설문 문항 코드(예: `E1A-1`)도 노출하지 않는다 — 이런 코드는 애초에 프롬프트에 넣기 전에 서버에서 제거함 ([12.12](#1212-값-안에-내장된-내부-코드가-그대로-노출된-문제) 참고)
...
## 12.9 종료했다고 생각한 개발 서버가 남아 옛날 코드를 계속 서빙한 문제
...
## 12.10 대화 히스토리 도입 후 없는 데이터 종류를 형식만 보고 지어내는 문제
...
## 12.11 국가별 데이터 행 수 상한(LIMIT)이 큰 국가의 데이터를 조용히 잘라버린 문제

#### 현상

위 12.10을 조사하는 과정에서, AI가 "지어냈다"고 자백한 항목 중 일부(도시별 방문 의향, 콘텐츠 이용 빈도 등)가 실제로는 해당 조사에 존재하는 항목이라는 것을 DB에서 직접 확인했습니다.
...
## 12.12 값 안에 내장된 내부 코드가 그대로 노출된 문제
...
- 멀티턴 대화 기억을 추가하자 새로운 종류의 문제가 따라왔습니다 — 이전 turn의 진짜 답변이 프롬프트에 그대로 남아있으니, 모델이 그 형식을 다른 국가/다른 turn에도 억지로 재현하려는 경향이 생겼습니다([12.10](#1210-대화-히스토리-도입-후-없는-데이터-종류를-형식만-보고-지어내는-문제)). 기능 하나(대화 기억)를 추가하면 그게 다른 안전장치(그라운딩 규칙)와 상호작용해 새 실패 유형을 만들 수 있다는 것을 체감했습니다.
