...
- [2025 잠재방한여행객조사] 관련 답변은 인용한 수치마다 국가/항목을 구조화된 `citations`로 함께 받아, 서버가 실제 데이터와 자동 대조한다 — 여러 진짜 값을 조합해 존재하지 않는 통계를 합성하는 유형의 환각까지 잡기 위함 ([12.15](#1215-숫자가-데이터에-있는지만으로는-합성형-환각을-못-잡는-문제--구조화된-인용으로-전환) 참고)
...
## 12.15 "숫자가 데이터에 있는지"만으로는 합성형 환각을 못 잡는 문제 — 구조화된 인용으로 전환

#### 현상

12.9/12.10에서 "없는 데이터를 언급하는" 유형과 "잘려서 안 보이는" 유형은 고쳤지만, 진짜 값 여러 개를 조합해 존재하지 않는 새 통계(예: "패키지투어 선호자 138명, 픽업 서비스 48.0%")를 만들어내는 유형은 남아있었습니다. 답변에 나온 숫자가 프롬프트에 주입한 데이터 어딘가에 있기만 하면 통과하는 검증 방식으로는, 숫자 자체는 실존해도 그 조합/문맥이 지어진 경우를 구분할 수 없었습니다.

#### 원인

숫자의 "존재 여부"만 보는 검증은 원천적으로 문맥을 못 봅니다. 어느 국가·어느 항목에서 나온 값인지까지 확인하려면, 모델이 숫자만 던지는 게 아니라 **그 값의 출처(국가/항목)를 구조화된 형태로 같이 내놓게** 만들고, 그 출처를 코드로 직접 조회해서 대조해야 합니다.

#### 해결 (검증 과정)

원본 코드를 바로 고치지 않고 `backend/experiments/verify_grounding_poc.py`라는 독립 스크립트로 먼저 검증했습니다:
1. 응답 형식에 `citations: [{"country", "item", "value"}]`를 추가하도록 요청하고, 실제 DB의 `potential_tourist_2025_survey` 데이터로 라이브 테스트.
2. 1차(country/topic/item 완전일치)에서는 실제로 맞는 인용도 다수 오탐 처리됐습니다 — 모델이 긴 item 라벨을 줄여 쓰거나(`"...- 전문 가이드 동반"` → `"전문 가이드 동반"`), 원본 데이터 자체의 topic 표기가 항목마다 일관되지 않아서(`"(~2028년)"` 유무, topic이 비어있는 행 등) 벌어진 문제였습니다.
3. topic 조건을 빼고 item을 정규화(공백·기호 제거) 후 부분일치로, 같은 키에 값이 여러 개면 후보 전체 중 하나만 맞아도 통과하도록 완화 — 그 결과 실제 사례 3건(정상 질문 1건 + 과거 사고 재현 2건) 전부 오탐 없이 통과했고, 고의로 지어낸 인용 2건(완전 창작 항목 + 값 조작)은 여전히 정확히 걸러지는 것을 확인한 뒤에야 원본에 반영했습니다.

`chat_service.py`의 규칙 19번(citations 요구)과 `_verify_survey_citations()`로 반영했습니다. citations가 실제 데이터와 하나라도 안 맞으면 한 번 재시도하고, 재시도도 실패하면 답변은 그대로 보여주되 끝에 자동 검증 실패 경고를 붙입니다(오탐으로 정상 답변을 통째로 막는 것보다, 경고를 붙이고 보여주는 편이 더 안전하다고 판단).

#### 검증 (원본 반영 후 실제 `/api/chat`으로 심층 테스트)

PoC가 아니라 실제로 반영된 `chat_service.py`를 대상으로, 의도적으로 더 까다로운 질문 4개를 던져 DB 원본과 대조했습니다.

| 질문 유형 | 결과 |
|---|---|
| UAE·사우디 두 나라 비교 (패키지투어 서비스) | 인용된 수치 전부 DB와 정확히 일치 |
| 대화 히스토리 활용 후속 질문("위 내용 참고해서 일본은?") | 새 국가(일본) 데이터를 다시 조회해 정확히 답변 — 12.9에서 겪었던 "이전 턴 형식 모방" 재발 없음 |
| 데이터가 답할 수 없는 교차통계 유도("픽업 서비스와 할랄 식당 정보를 둘 다 원하는 사람은 몇 명?") | 지어내지 않고 "두 조건을 교차한 데이터가 없다"고 명시하며 거절, 각 개별 수치만 따로 인용 |
| "숫자 하나도 빠뜨리지 말고" 전량 나열 압박 (소표본 국가 몽골) | 17개 수치 전부 DB와 정확히 일치, 데이터에 없는 "이유"는 없다고 솔직히 답변 |

4건 모두 자동 검증 경고(⚠️) 없이 통과했고, DB에 직접 대조한 수치도 전부 일치했습니다. 특히 세 번째·네 번째 케이스처럼 "답을 짜내도록 압박하는" 질문에서 모델이 스스로 한계를 인정한 것은, citations 검증이 도입되기 전이었다면 합성형 환각이 나오기 쉬웠던 유형입니다.

#### 남은 한계

country/item/value만 확인하고 topic(어느 하위 질문 그룹인지)은 검증하지 않기 때문에, 같은 국가 안에서 값을 엉뚱한 하위 항목에 잘못 붙이는 실수는 여전히 못 잡습니다. 또한 이 검증은 [2025 잠재방한여행객조사] 블록에만 적용되어 있고, 나머지 데이터 블록(Gap/장벽 등)에는 아직 적용하지 않았습니다.
...
- citations 기반 검증을 [2025 잠재방한여행객조사] 외 나머지 데이터 블록(Gap/장벽 등)으로 확장 — 지금은 12.15에서 반영한 검증이 이 블록 하나에만 적용되어 있음
- citations 검증에 topic(하위 질문 그룹) 일치까지 포함 — 지금은 country+item+value만 보고 있어, 같은 국가 안에서 값이 엉뚱한 하위 항목에 잘못 붙는 경우는 못 잡음(12.15 "남은 한계" 참고)
