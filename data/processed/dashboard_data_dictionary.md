...
**주의**:
- **국가 범위가 다르다**: 이 표는 한류실태조사 대상 **30개국** 전체다. 나머지 표들의 23개국(잠재방한객조사 고정 목록)과 다르며, 30개국에는 아르헨티나·칠레·이탈리아·스페인·이집트·폴란드·남아프리카공화국이 추가로 포함된다. `country_profile_base` 등과 country로 직접 조인하면 7개국이 누락되니 주의.
- 1-17/1-18의 BASE는 "방문 비의향자"가 아니라 **해당 콘텐츠 카테고리 경험/인지자**다(barrier_pattern_analysis.csv와 BASE가 다름 — 직접 차감하거나 같은 모집단처럼 섞지 않는다).

## 15. `potential_tourist_2025_survey.csv` (39,667행, C급) — 2025 잠재방한여행객조사 세그먼트 데이터

2026-08-26 추가. `2025_잠재방한여행객조사_분석.pdf`(434페이지, 텍스트 PDF — OCR 아님,
`pymupdf` 표 인식으로 추출, 검증 이슈 0건)에서 핵심 질문과 직결되는 118페이지만 선별
추출한 롱포맷 데이터. 8개 장벽을 성별/연령별/거주국별로 쪼갠 세그먼트 버전과, 희망
지출/숙박/쇼핑/결제/스마트폰/참여활동(의료관광·한류콘텐츠체험 포함) 등 사업기회
데이터를 담고 있다. 어떤 페이지를 포함/제외했는지는 `data/scripts/README.md` "2025
잠재방한여행객조사 추출" 절 참고.

| 컬럼 | 타입 | 의미 |
|---|---|---|
| survey | string | "일반외국인"(n=16,360) / "방한의향자"(그 중 방한의향자 서브셋) |
| page | int | 원본 PDF 페이지 |
| topic | string | 표 제목 원문(예: "24. 향후 3년 내 한국여행 비의향 및 미결정 이유 : 불편한 언어소통") |
| base_desc | string | 표에 명시된 BASE/단위 설명 원문 |
| group | string | 세그먼트 그룹("전체"/"성별"/"연령별"/"성별&연령별"/"거주국별") |
| segment | string | 그룹 내 세부 값(예: "남성", "20대", "미국") |
| sample_n | float, nullable | 세그먼트별 사례수. 표가 세그먼트마다 표본수를 표기하는 경우만 채워지고, 블록 전체 고정값(예: "n=586")으로만 표기된 경우는 비어있음 — 그 경우 `item`/`topic` 텍스트에 원문 그대로 남아있으니 참고 |
| item | string | 응답 항목 텍스트(원문 그대로, OCR 아님 — 정규화 불필요) |
| value | float | 응답 비율(%) 또는 점수 |

**주의**:
- **국가 범위가 세 번째로 다르다**: 이 조사는 **26개국**이며, `country_profile_base`의
  23개국(잠재방한객, 다른 회차/연도), `content_liking_disliking_reasons`의 30개국
  (한류실태)과 또 다른 리스트다. 홍콩(중국)·몽골·네덜란드·브라질 등 구성이 다르므로
  `segment`를 country로 조인하기 전 반드시 확인한다.
- `topic`에 "24. ... : 불편한 언어소통"처럼 barrier_pattern_analysis.csv의 8개 장벽과
  이름이 겹치는 항목이 있는데, **BASE와 조사 자체가 다르다**(이 조사는 2025년, 별도
  회차). 같은 지표의 반복측정처럼 직접 비교하지 않는다 — "다른 조사에서도 유사한
  경향이 관찰된다" 정도로만 참고한다.
